...
export default function AgentPage({ params }: { params: Promise<{ id: string; agentId: string }> }) {
  const resolvedParams = use(params);
  const { id: projectId, agentId } = resolvedParams;
  const agentName = AGENT_NAMES[agentId] || "AI Agent";
  
  const { data: conversations, isLoading: isLoadingSidebar, refetch: refetchSidebar } = useCopilot(projectId, agentId);
  ...
  const handleSendMessage = async (content: string) => {
    ...
    if (!targetConversationId) {
      try {
        const newConv = await CopilotService.createConversation(projectId, agentId);
      ...
      <div className="w-full h-[calc(100vh-80px)] max-h-[calc(100vh-80px)] p-0 sm:p-4 md:p-6 pb-20 sm:pb-6 flex flex-col">
        <div className="flex items-center mb-4">
          <Link 
            ...
            className="inline-flex items-center text-xs font-medium text-text-secondary hover:text-text-primary transition-colors focus:outline-none focus:ring-1 focus:ring-primary rounded px-1 -ml-1 shrink-0"
          >
        ...
        <div className="flex-1 overflow-hidden border border-border rounded-xl shadow-sm">
          <ChatLayout
            sidebar={
              <ConversationSidebar 
